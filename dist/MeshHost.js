...
class MeshHost extends EventEmitter {
  constructor(options) {
    ...
    _defineProperty(this, "_dataConnectionPingMap", {});
    ...
    _defineProperty(this, "_isOwner", false);

    _defineProperty(this, "setOwnerId", id => {
      this._isOwner = id;
    });
    ...
    _defineProperty(this, "_healtCheckInterval", false);

    _defineProperty(this, "_healthCheckPeers", () => {
    ...
    _defineProperty(this, "_peerDropped", id => {
    ...
    _defineProperty(this, "_hostOwnerInterval", false);
    ...
    _defineProperty(this, "_listenDataConnection", dc => {
      ...
      dc.on("open", async () => {
        ...
        if (this._initData) {
          if (this.options.owner_mode) {
            let owner_init_data = {};
            if (owner_init_data[this._isOwner]) owner_init_data[this._isOwner] = this._initData[this._isOwner];
            dc.send({
        ...
        if (this.options.mesh_limit === -1) {
          if (this.options.owner_mode) {
            if (this._isOwner !== dc.peer) this._dataConnectionMap[this._isOwner] && this._dataConnectionMap[this._isOwner].send({
              "identify": dc.peer
            });
          } else {
            Object.keys(this._dataConnectionMap).forEach(key => {
          ...
          if (Object.keys(this._dataConnectionMap).length < this.options.mesh_limit) {
            ...
            dc.send({
              "meshlimit": this.options.mesh_limit
            }); // dc.close() 
