...
const mesh = (room, options = {}) => {
  ...
  if (!options.insert_dummy_track) {
    //there is a issue, if support during starting of call we don't have audio/video from media stream
    //due to any reason. and then we start screen sharing.  or turn on audio later on.
    // this will fails because our existing does replaceTrack and there is no track to replace!
    // so we isnert a dummy audio/video track in such cases!
    // this can also help to connect with a call without any stream at all!
    options.insert_dummy_track = true;
  }
  ...
  let network = new _MeshNetwork.default(room, options);
  network.addPeer();
  return network;
};
